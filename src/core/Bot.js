        ...
        this.bot.on('open', (account) => {
            ...
            this.bot.ws.ev.on('messages.upsert', ({ messages, type }) => {
                ...
                for (const m of messages) {
                    messageHandler.handleMessage(this.bot, m).catch(err => logger.error('Error processing message:', err));
                }
            });
            this.bot.ws.ev.on('group-participants.update', (event) => {
                clearPermissionCache(event.id);
                welcomeHandler.handle(this.bot, event).catch(err => logger.error('Error in welcome handler:', err));
                alertHandler.handle(this.bot, event).catch(err => logger.error('Error in alert handler:', err));
            });
        });
        this.bot.on('close', (reason) => logger.info(`ꕢ Conexión cerrada: ${reason}`));
        this.bot.on('error', (err) => logger.error('ꕢ Error del bot:', err));
    }
