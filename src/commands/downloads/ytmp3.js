...
async function ytMp3(url) {
    try {
        ...
        if (data && data.download_url) return { media: { audio: data.download_url }, title: data.title || 'Audio', author: { name: data.author || 'Desconocido' }, duration: data.duration || '', cover: data.thumbnail || '' };
    ...
    async execute(ctx) {
        ...
        try {
            ...
            if (info && info.media && info.media.audio) {
                ...
                await ctx.bot.sendMessage(ctx.chatId, { audio: { stream: audioStream.data }, mimetype: 'audio/mpeg', fileName: `${info.title.replace(/[\/\\:*?"<>|]/g, '_')}.mp3`, contextInfo: { externalAdReply: { title: info.title, body: info.author?.name || 'YouTube', thumbnailUrl: info.cover || '', sourceUrl: ctx.args[0], mediaType: 1, renderLargerThumbnail: true } } }, { quoted: ctx.msg });
