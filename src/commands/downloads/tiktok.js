...
export default {
    ...
    async execute(ctx) {
        ...
        const links = m.message?.conversation?.match(/https?:\/\/(www|vt|vm|t)?\.?tiktok\.com\/\S+/g) || m.message?.extendedTextMessage?.text?.match(/https?:\/\/(www|vt|vm|t)?\.?tiktok\.com\/\S+/g) || args.filter(arg => /https?:\/\/(www|vt|vm|t)?\.?tiktok\.com\/\S+/.test(arg));
        if (!links || links.length === 0) return await bot.sendMessage(chatId, { text: styleText(`《✧》 *Uso incorrecto del comando*\n\n*Ejemplos:*\n✿ #tiktok https://www.tiktok.com/@user/video/xxx`) });
        const memCheck = global.memoryManager?.canProcessDownload();
        if (memCheck && !memCheck.allowed) return await bot.sendMessage(chatId, { text: styleText(memCheck.message) });
        ...
        for (const link of linksToProcess) {
            try {
                ...
                if (data.images?.length) {
                    ...
                    if (data.images.length > maxImages) await bot.sendMessage(chatId, { text: styleText(`⚠️ Solo se mostraron ${maxImages} de ${data.images.length} imágenes.`) });
                } else if (data.play) {
