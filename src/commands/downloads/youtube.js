...
export default {
    ...
    async execute(ctx) {
        try {
            ...
            if (!url.includes('youtube.com') && !url.includes('youtu.be')) return await ctx.reply(styleText('《✧》 Por favor ingresa un link válido de YouTube.'));
            await ctx.reply(styleText(`Usa:\n• #ytmp3 ${url} para audio\n• #ytmp4 ${url} para video`));
        } catch (error) {
