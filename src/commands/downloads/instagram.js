...
export default {
    ...
    async execute(ctx) {
        try {
            if (ctx.args.length === 0) return await ctx.reply(styleText(`ꕢ *Uso incorrecto del comando*\n\nEjemplo:\n> #instagram https://www.instagram.com/p/xxxxx\n> #ig https://www.instagram.com/reel/xxxxx`));
            ...
            if (!response.data || response.data.length === 0) return await ctx.reply(styleText('ꕢ No se encontró contenido en este enlace.\n\n> *Tip:* Verifica que el enlace sea correcto y público.'));
            const media = response.data.sort((a, b) => parseInt(b.resolution || '0') - parseInt(a.resolution || '0'))[0];
            if (!media || !media.url) throw new Error('No se encontró un medio válido.');
            await ctx.replyWithVideo(media.url, { caption: styleText(`ꕣ *Instagram Downloader*\n\n> ✿ *Resolución* » ${media.resolution || 'Desconocida'}\n> ✿ *Link original* » ${url}`) });
        } catch (error) {
