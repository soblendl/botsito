...
export default {
    ...
    async execute(ctx) {
        ...
        if (!botAdmin) {
            return await ctx.reply(styleText('ꕢ Necesito ser administrador para degradar usuarios.'));
        }
        const mentions = extractMentions(ctx);
        if (mentions.length === 0) {
            return await ctx.reply(styleText('ꕢ Debes mencionar al usuario a degradar.'));
        }
        try {
            ...
            await ctx.reply(styleText(`ꕣ @${participantIds[0].split('@')[0]} ya no es administrador.`), {
