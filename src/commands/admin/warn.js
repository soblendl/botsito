...
export default {
    ...
    async execute(ctx) {
        ...
        if (await isAdmin(ctx.bot.sock, ctx.chatId, targetUser)) return await ctx.reply(styleText('ꕢ No puedo advertir a un administrador.'));
        if (targetUser.includes(ctx.bot.sock.user.id.split(':')[0])) return await ctx.reply(styleText('ꕢ No puedo advertirme a mí mismo.'));
        ...
        if (warns >= 3) {
            if (await isBotAdmin(ctx.bot.sock, ctx.chatId)) {
                ...
                try {
                    ...
                    userData.warns = 0;
                    ctx.dbService.markDirty();
                } catch (error) { await ctx.reply(styleText('ꕢ Error al intentar expulsar al usuario.')); }
            } else {
