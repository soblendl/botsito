import { formatNumber, styleText } from '../../utils/helpers.js';
export default {
    commands: ['slot', 'slots', 'tragaperras'],
    category: 'economy',
    async execute(ctx) {
        if (ctx.isGroup && !ctx.dbService.getGroup(ctx.chatId).settings.economy) return await ctx.reply(styleText('ꕢ El sistema de economía está desactivado en este grupo.'));
        const userData = ctx.dbService.getUser(ctx.sender);
        const userEconomics = userData.economy || { coins: 0 };
        let amount = parseInt(ctx.args[0]);
        if (!amount || isNaN(amount) || amount <= 0) return await ctx.reply(styleText('ꕢ Uso: #slot <cantidad>\n> Ejemplo: * #slot* 100'));
        if (amount > userEconomics.coins) return await ctx.reply(styleText('ꕢ No tienes suficientes coins.'));
        const emojis = ['🍇', '🍉', '🍊', '🍋', '🍌', '🍍', '🍒', '🍓'];
        const a = emojis[Math.floor(Math.random() * emojis.length)];
        const b = emojis[Math.floor(Math.random() * emojis.length)];
        const c = emojis[Math.floor(Math.random() * emojis.length)];
        let result = '';
        let winAmount = 0;
        let newBalance = userEconomics.coins - amount;
        if (a === b && b === c) {
            winAmount = amount * 5;
            newBalance += winAmount;
            result = `ꕣ *JACKPOT!!!* \n\n> ➵Ganaste *${formatNumber(winAmount)}* coins (x5)`;
        } else if (a === b || b === c || a === c) {
            winAmount = Math.floor(amount * 1.5);
            newBalance += winAmount;
            result = `ꕣ *¡Ganaste!* \n\n> ✐ Ganaste *${formatNumber(winAmount)}* coins (x1.5)`;
        } else {
            result = `ꕢ *Perdiste* \n\n> ✐ Perdiste *${formatNumber(amount)}* coins`;
        }
        ctx.dbService.updateUser(ctx.sender, { 'economy.coins': newBalance });
        await ctx.reply(styleText(`ꕣ *SLOTS* 🎰\n\n❐ | ${a} | ${b} | ${c} | 👈\n\n${result}\n> Nuevo balance: ${formatNumber(newBalance)} coins`));
    }
};
