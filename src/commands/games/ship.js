...
export default {
    ...
    async execute(ctx) {
        ...
        let person1 = sender;
        let person2 = null;
        if (mentioned.length >= 2) { person1 = mentioned[0]; person2 = mentioned[1]; }
        else if (mentioned.length === 1) person2 = mentioned[0];
        else if (quoted) person2 = quoted;
        if (!person2) return await ctx.reply(styleText('ꕣ *SHIP - Compatibilidad de Amor*\n\nMenciona a 2 personas o responde a alguien:\n> • #ship @persona1 @persona2\n> • Responder + #ship\n> • #ship @persona'));
        ...
        const percentage = Math.abs((seed * 12345) % 101);
        ...
        await ctx.bot.sendMessage(chatId, { text: styleText(text), mentions: [person1, person2] }, { quoted: msg });
    }
};
