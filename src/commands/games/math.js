...
export default {
    ...
    async before(ctx) {
        ...
        if (!isNaN(answer) && answer === game.answer) {
            ...
            dbService.updateUser(sender, { coins: (userData.coins || 0) + reward });
    ...
    async execute(ctx) {
        ...
        else if (op === '-') { a = Math.floor(Math.random() * 50) + 20; b = Math.floor(Math.random() * 20); answer = a - b; }
        else if (op === '*') { a = Math.floor(Math.random() * 10) + 1; b = Math.floor(Math.random() * 10) + 1; answer = a * b; }
        else if (op === '/') { b = Math.floor(Math.random() * 10) + 1; answer = Math.floor(Math.random() * 10) + 1; a = answer * b; }
        const timer = setTimeout(() => { if (mathGames.has(chatId)) { ctx.reply(styleText(`ꕢ *Tiempo agotado* La respuesta era: ${answer}`)); mathGames.delete(chatId); } }, 30000);
