    ...
    async execute(ctx) {
        ...
        if (command === 'setbirth') {
            ...
            if (!dateRegex.test(birth)) {
                return await ctx.reply(styleText('ꕢ Formato inválido. Usa DD/MM/YYYY (Ej: 01/01/2000)'));
            }
            await dbService.updateUser(sender, { 'profile.birthday': birth });
            return await ctx.reply(styleText(`ꕣ Cumpleaños establecido a: ${birth}`));
        }

        if (command === 'setgen') {
            const gen = args[0]?.toLowerCase();
            if (!['m', 'f'].includes(gen)) {
                return await ctx.reply(styleText('ꕢ Género inválido. Usa *m* (Masculino) o *f* (Femenino).'));
            }
            const gender = gen === 'm' ? 'Masculino' : 'Femenino';
            await dbService.updateUser(sender, { 'profile.gender': gender });
            return await ctx.reply(styleText(`ꕣ Género establecido a: ${gender}`));
        }
        ...
        const xpForCurrentLevel = Math.pow(level - 1, 2) * 100; // Fixed: level - 1
