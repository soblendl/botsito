...
export default {
    ...
    async execute(ctx) {
        ...
        if (playerScore === 21 && dealerScore === 21) {
            await sendTable(ctx, game, true);
            await finishGame(ctx, game, 'tie', 'ꕢ Ambos tienen Blackjack. Empate.');
            return;
        } else if (playerScore === 21) {
            await sendTable(ctx, game, true);
            await finishGame(ctx, game, 'blackjack', 'ꕢ ¡BLACKJACK NATURAL!');
            return;
        } else if (dealerScore === 21) {
            await sendTable(ctx, game, true);
            await finishGame(ctx, game, 'lose', 'ꕢ El dealer tiene Blackjack.');
            return;
        } else {
