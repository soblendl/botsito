    ...
    async processNext() {
        ...
        while (this.jobs.length > 0) {
            ...
            try {
                ...
                logger.error(`InMemoryQueue ${this.name} job error:`, err);
            }
        }
        this.processing = false;
    }
...
class QueueManager {
    ...
    async init() {
        try {
            ...
            this.useRedis = true;
            logger.info('📌 QueueManager: Redis disponible');
        } catch (err) {
            this.useRedis = false;
            logger.warn('⚠️ QueueManager: Redis no disponible, usando cola en memoria');
        }
    }
