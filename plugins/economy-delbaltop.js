    ...
    async execute(ctx) {
        ...
        if (!targetId) {
            return await ctx.reply(styleText(
                'ꕥ *DELBALTOP - Eliminar de Ranking*\n\n' +
        ...
        try {
            ...
            if (success) {
                await ctx.reply(styleText(`✅ Usuario \`${targetId}\` eliminado de la base de datos y del ranking.`));
            } else {
                await ctx.reply(styleText(`⚠️ No se encontró al usuario \`${targetId}\` o no se pudo eliminar.`));
            }
        } catch (error) {
            console.error('Error en delbaltop:', error);
            await ctx.reply(styleText(`❌ Error al intentar eliminar: ${error.message}`));
        }
    }
};
